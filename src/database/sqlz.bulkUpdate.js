 ...
 * Model.bulkUpdate(values[, options])
    values: (Required) List of values to update with the key (primary key or othwerwise)
    options: (Optional)
    options.key: STRING -> The name of the field to use as the key to match the rows to update. default: id
    options.fields: [STRING] -> The list of fields to update when values contains more field than needed. Only the fields in this list will be updated. default: all fields
